...
mod tests {
    ...
    use silentpayments::utils::{get_pubkey_from_input, is_p2tr, VinData, LabelHash,};
    ...
    fn process_test_case(test_case: TestData) {
        ...
        for sendingtest in test_case.sending {
            ...
            for input in given.vin {
                ...
                let prevout = hex::decode(&input.prevout.scriptPubKey.hex).unwrap();
                ...
                match get_pubkey_from_input(&vin_data) {
                    Ok(_pubkey) => {
                        match _pubkey {
                            Some(_pubkey) => tmp_input_priv_keys.push((SecretKey::from_str(&input.private_key).unwrap(), is_p2tr(&vin_data.prevout))),
                            None => continue,
                        }
                    },
